 ...
 */
public final class GridFormatValidator {

    private GridFormatValidator() {
    }
     ...
     * @return returns false if a character is not valid
     * in the array otherwise true
     */
    public static boolean isValidFormat(
            final char[] row,
            final char[] availableCharacter
        ...
        for (int index = 0; index < length; index++) {
            if (
                    !Arrays.toString(availableCharacter)
                    .contains("" + row[index])
            ) {
                return false;
            }
     ...
     * @param grid two-dimensional character array
     * @return returns false if the size of the children
     * is not equal to the parent, otherwise true.
     */
    public static boolean isUniform(final char[][] grid) {
        int parentLength = grid.length;
        for (int y = 0; y < parentLength; y++) {
            if (!isUniform(grid[y], parentLength)) {
                return false;
     ...
     * is not equal to the parent, otherwise true.
     */
    private static boolean isUniform(final char[] row, final int parentLength) {
     ...
     */
    public static boolean isValidSize(final int length) {
        return length == GridSizeEnum.NORMAL_GRID_SIZE.getValue()
                || length == GridSizeEnum.LARGE_GRID_SIZE.getValue();
    }
}
